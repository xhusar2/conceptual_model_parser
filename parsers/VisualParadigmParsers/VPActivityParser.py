...
class VPActivityParser(ActivityDiagramParser):
    ...
    def parse_initial_nodes(self, model, namespaces):
        ...
        for initial in initial_nodes:
            m_initial_nodes.add(self.parse_node(initial, namespaces, "Initial"))
    ...
    def parse_activity_finals(self, model, namespaces):
        ...
        for activity_final in activity_finals:
            ...
            if final_type is not None:
                m_activity_finals.add(self.parse_node(activity_final, namespaces, "ActivityFinal"))
    ...
    def parse_flow_finals(self, model, namespaces):
        ...
        for activity_final in flow_finals:
            ...
            if final_type is not None:
                m_flow_finals.add(self.parse_node(activity_final, namespaces, "FlowFinal"))
    ...
    def parse_forks_joins(self, model, namespaces):
        ...
        for fork_join in forks_joins:
            m_forks_joins.add(self.parse_node(fork_join, namespaces, "ForkJoin"))
    ...
    def parse_decisions_merges(self, model, namespaces):
        ...
        for decision_merge in decisions_merges:
            m_decisions_merges.add(self.parse_node(decision_merge, namespaces, "DecisionMerge"))
        return m_decisions_merges

    @staticmethod
    def parse_source_target_relation(relation, namespaces, rel_type):
    ...
    def parse_control_flows(self, model, namespaces):
        ...
        for control_flow in control_flows:
            m_control_flows.add(self.parse_source_target_relation(control_flow, namespaces, "ControlFlow"))
    ...
    def parse_object_flows(self, model, namespaces):
        ...
        for object_flow in object_flows:
            m_object_flows.add(self.parse_source_target_relation(object_flow, namespaces, "ObjectFlow"))
        return m_object_flows

    #TODO check if not in model
    def parse_partitions(self, model, namespaces):
        m_partitions = set()
        partitions = model.findall('.//ownedMember[@xmi:type="uml:ActivityPartition"]', namespaces)
        for partition in partitions:
            m_partitions.add(self.parse_node(partition, namespaces, "Partition"))
        return m_partitions

    #TODO check if not in model partition
    ...
    def parse_central_buffers(self, model, namespaces):
        ...
        for central_buffer in central_buffers:
            m_central_buffers.add(self.parse_node(central_buffer, namespaces, "CentralBuffer"))
    ...
    def parse_input_pins(self, model, namespaces):
        ...
        for input_pin in input_pins:
            m_input_pins.add(self.parse_node(input_pin, namespaces, "InputPin"))
        return m_input_pins

    #TODO check if not in model
    def parse_output_pins(self, model, namespaces):
        m_output_pins = set()
        output_pins = model.findall('.//result[@xmi:type="uml:OutputPin"]', namespaces)
        for output_pin in output_pins:
            m_output_pins.add(self.parse_node(output_pin, namespaces, "OutputPin"))
